...
bool isForwardableSuperCommand(const planner_msgs::SuperPositionCommand::ConstPtr& msg) {
    // SUPER 当前把主轨迹标成 READY，把 backup trajectory 标成 EMER。
    // 这两种状态都属于可以交给控制器继续执行的有效轨迹。
    return msg->trajectory_flag == planner_msgs::SuperPositionCommand::TRAJECTORY_STATUS_READY ||
           msg->trajectory_flag == planner_msgs::SuperPositionCommand::TRAJECTORY_STATUS_EMER;
}

void superPositionCallback(const planner_msgs::SuperPositionCommand::ConstPtr& msg) {
    if (g_ready_only && !isForwardableSuperCommand(msg)) {
        return;
    }

    sunray_msgs::UAVControlCMD control_cmd;

    // 保留时间戳和坐标系信息，方便控制器和调试工具对齐。
    ...
    // SUPER 的位置、速度、加速度、jerk 可直接映射到控制器输入。
    ...
    // 两种偏航输入都填好，最终由 yaw_mode 决定控制器使用哪一组。
...
int main(int argc, char** argv) {
    ros::init(argc, argv, "Super2Sunray_node");
    ...
    ros::Subscriber super_cmd_sub = nh.subscribe(input_topic, queue_size, superPositionCallback);

    // 保留订阅对象，避免离开作用域后析构导致回调失效。
    (void)super_cmd_sub;

    ROS_INFO_STREAM("Super2Sunray subscribe: " << input_topic);
    ROS_INFO_STREAM("Super2Sunray publish:   " << output_topic);
    ROS_INFO_STREAM("Super2Sunray use_yaw_rate: " << (g_use_yaw_rate ? "true" : "false"));
    ROS_INFO_STREAM("Super2Sunray ready_only:   " << (g_ready_only ? "true" : "false"));
