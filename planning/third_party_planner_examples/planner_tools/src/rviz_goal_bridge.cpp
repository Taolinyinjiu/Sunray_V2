...
bool transform_pose_to_target_frame(const geometry_msgs::PoseStamped& input_pose,
    ...
    if (input_frame.empty()) {
        SUNRAY_WARN("[planner_tools] {} rejected: empty frame_id", source_tag);
        return false;
    }
    if (normalized_target_frame.empty()) {
        SUNRAY_WARN("[planner_tools] {} rejected: empty target_frame_id", source_tag);
        return false;
    }
    ...
    if (!context.tf_buffer.canTransform(
        ...
        SUNRAY_WARN("[planner_tools] {} cannot transform {} -> {} within {:.3f}s",
                    source_tag,
                    input_frame,
    ...
    try {
        ...
        SUNRAY_WARN("[planner_tools] {} transform failed: {} -> {} error={}",
                    source_tag,
                    input_frame,
                    normalized_target_frame,
                    ex.what());
...
bool resolve_goal_command(const geometry_msgs::PoseStamped& input_goal,
    ...
    if (input_frame.empty()) {
        SUNRAY_WARN("[planner_tools] goal bridge rejected goal from {}: empty frame_id",
    ...
    if (input_frame != kWorldFrameId) {
        SUNRAY_WARN("[planner_tools] goal bridge rejected unsupported frame_id={} from {}, supported=[{},{},{}]",
                    input_frame,
                    context.config.rviz_topic,
                    context.config.local_frame_id,
...
bool resolve_goal_height(const GoalPlanMode plan_mode,
    ...
    if (!context.has_agent_odom) {
        SUNRAY_WARN("[planner_tools] goal bridge cannot use agent height because odom has not been received yet from {}",
...
void handle_goal(const geometry_msgs::PoseStamped::ConstPtr& msg, BridgeContext* context) {
    ...
    SUNRAY_INFO("[planner_tools] goal bridge forwarded goal: {} -> {} cmd={} pos=({:.3f}, {:.3f}, {:.3f}) yaw={:.3f} topic={}",
                msg->header.frame_id,
...
int main(int argc, char** argv) {
    ...
    try {
        ...
        SUNRAY_INFO("[planner_tools] pose goal bridge: rviz_topic={} planning_cmd_topic={} local_frame={} global_frame={} world_goal_plan_frame={} use_agent_height={} fixed_goal_height={} odom_topic={}",
                    context.config.rviz_topic,
                    context.planning_cmd_topic,
                    context.config.local_frame_id,
                    context.config.global_frame_id,
        ...
        ROS_ERROR("[planner_tools] failed to start rviz_goal_bridge: %s", ex.what());
        return 1;
    }
}
