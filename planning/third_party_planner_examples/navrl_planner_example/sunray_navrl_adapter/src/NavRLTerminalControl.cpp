...
本程序功能：
    ...
    2. 保留 Sunray TAKEOFF / LAND / HOVER / MOVE_POINT 控制指令发布功能。
    3. NAVRL_GOAL 只需要用户输入 x、y；MOVE_POINT 需要输入完整位置和偏航角。
*/
...
void printMenu(const std::string& goal_topic,
    ...
    std::cout << kDim << " 说明：位置单位为 m；输入 yaw 使用 deg，发布到控制指令时自动转换为 rad。"
              << kReset << std::endl;
    std::cout << kDim << " 规则：TAKEOFF / LAND / HOVER / MOVE_POINT 都只发布一次。"
              << kReset << std::endl;
    ...
    std::cout << kCyan << " 5 " << kReset << commandLabel(5)
              << kDim << "    输入 x y z yaw，发布 Sunray control_cmd" << kReset << std::endl;
...
int main(int argc, char** argv) {
    ...
    control_topic = "/" + agent_name + std::to_string(uav_id) + "/sunray/uav_control/control_cmd";
    ...
    while (ros::ok()) {
        ...
        if (selection == 1) {
            ...
            uint8_t control_cmd = sunray_msgs::UAVControlCMD::UNDEFINE;
            ...
            double z = 0.0;
            double yaw_deg = 0.0;
            if (!readPoint(*input, "MOVE_POINT", x, y, z, yaw_deg)) {
                if (input->eof()) {
                    break;
                }
                continue;
            ...
            sunray_msgs::UAVControlCMD cmd = makeBaseControlCmd(sunray_msgs::UAVControlCMD::MOVE_POINT, frame_id);
            cmd.desired_pos.x = x;
            cmd.desired_pos.y = y;
            cmd.desired_pos.z = z;
