...
std::string g_output_topic = "/uav1/sunray/uav_control/control_cmd";
...
void navrlCmdVelCallback(const geometry_msgs::TwistStamped::ConstPtr& msg) {
    ...
    control_cmd.control_cmd = sunray_msgs::UAVControlCMD::MOVE_VELOCITY;

    control_cmd.desired_vel.x = msg->twist.linear.x;
    control_cmd.desired_vel.y = msg->twist.linear.y;
    control_cmd.desired_vel.z = 0.0;
    control_cmd.fixed_height = static_cast<float>(g_fixed_height);
...
void navrlSetpointPoseCallback(const geometry_msgs::PoseStamped::ConstPtr& msg) {
    ...
    control_cmd.cmd_source = sunray_msgs::UAVControlCMD::PLANNING;
    control_cmd.control_cmd = sunray_msgs::UAVControlCMD::MOVE_POINT;

    control_cmd.desired_pos.x = msg->pose.position.x;
    control_cmd.desired_pos.y = msg->pose.position.y;
    control_cmd.desired_pos.z = msg->pose.position.z;
