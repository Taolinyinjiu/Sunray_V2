...
<section id="super-planner">

## SUPER

`planning/source_planners/SUPER` 是第三方 SUPER 高速安全导航规划源码。Sunray 当前状态机中保留了 SUPER 类型枚举，但还没有接入可用 adapter；因此它目前更适合作为算法研究和后续适配对象。

### Sunray 中的位置

```text
planning/source_planners/SUPER/
├── mars_uav_sim
├── mission_planner
├── rog_map
├── scripts
...
```text
planning/source_planners/SUPER/misc/
```

手册页面不直接嵌入这些相对路径图片，避免迁移到网站后出现破图。

The upstream repository also provides a paper PDF under `planning/source_planners/SUPER/misc/scirobotics.ado6187.pdf`. If this repository supports your academic projects, please cite the upstream work.
...
Demo assets in the upstream source tree include `planning/source_planners/SUPER/misc/exp.gif` and `planning/source_planners/SUPER/misc/tracking.gif`.
...
Demo asset path: `planning/source_planners/SUPER/misc/tailsitter.gif`.
...
Image asset path: `planning/source_planners/SUPER/misc/image-20250130031404057.png`.
...
1. Place your **.pcd** file in `planning/source_planners/SUPER/mars_uav_sim/perfect_drone_sim/pcd/`.
2. Modify the `pcd_name` parameter in the corresponding YAML file under `planning/source_planners/SUPER/mars_uav_sim/perfect_drone_sim/config/`.
...
- `planning/source_planners/SUPER/super_planner/log/cmd_logs`
- `planning/source_planners/SUPER/super_planner/log/replan_logs`
...
- `planning/source_planners/SUPER/super_planner/Apps/read_replan_log.cpp`
- `planning/source_planners/SUPER/super_planner/Apps/traj_opt_tuning.cpp`
