...
```text
planning/third_party_planner_examples/ego_planner_example
```
...
| 路径 | 说明 |
| --- | --- |
| `sunray_ego_adapter/src/Ego2Sunray.cpp` | 把 EGO `position_cmd` 转为 `UAVControlCMD`。 |
| `sunray_ego_adapter/launch/Ego2Sunray.launch` | 桥接节点启动文件。 |
| `ego-planner-swarm` | EGO planner 示例源码，当前构建模块 `ego_planner_swarm` 指向这里。 |
| `ego-planner-swarm/plan_manage/launch/sim/run_gazebo_lio.launch` | 当前 Gazebo LIO 推荐入口。 |
| `../planner_msgs` | 示例规划器内部消息包，不要和 `planning/sunray_planner/sunray_planner_msgs` 混淆。 |
...
```bash
roslaunch planner_tools point_cloud_transform.launch \
...
`sunray_ego_adapter` 是 EGO 示例的配套桥接工具，不是 Sunray 主线规划工具。核心节点为：
...
```text
ego-planner-swarm/plan_manage/launch
```
...
- `planner_msgs` 是示例规划器内部消息，不要和 `planning/sunray_planner/sunray_planner_msgs` 混淆。
