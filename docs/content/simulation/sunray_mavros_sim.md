...
`/home/amov/Sunray_v2/sunray_mavros_sim` 是一个单节点 ROS 仿真包。它在一个 node 中完成 PCD 全局地图发布、MID360 风格局部点云仿真、UAV MAVROS setpoint 到电机转速的控制转换、四旋翼动力学、UGV 平面动力学、IMU、fake MAVROS 输出和 RViz 可视化。
...
命名语义：全局地图是环境级公共输出，默认发布到 `/sunray_mavros_sim/global_cloud`，不带智能体前缀；odom、IMU、局部点云和可视化是智能体级输出，使用 `~` 表示带 `/uav1` 或 `/ugv1` 这类前缀的相对话题。fake MAVROS 只给 UAV 链路使用。
...
```text
                         ...
                         +-----------------+
                                  |
                                  +--> /sunray_mavros_sim/global_cloud
                                  |
                                  v
                         +----------------------+
...
~/sunray_mavros_sim/odom
~/sunray_mavros_sim/cloud_world_frame
UAV 额外订阅：~/sunray_mavros_sim/cmd_RPM、~/mavros/state
       |
       v
+---------------+
| SimVisualizer |
+---------------+
       |
       +--> ~/sunray_mavros_sim/visualization
```
...
| 配置文件 | 对应模块 | 说明 |
| --- | --- | --- |
| `sunray_mavros_sim_node.yaml` | `sunray_mavros_sim_node` | 配置 `agent_name`、`agent_ids`、`global_frame_id` 和统一状态打印参数。 |
...
| `single_uav_simulator_config` | `$(find sunray_mavros_sim)/config/single_uav_simulator.yaml` |
...
| `ugv_simulator_config` | `$(find sunray_mavros_sim)/config/ugv_simulator.yaml` |
| `sim_visualizer_config` | `$(find sunray_mavros_sim)/config/sim_visualizer.yaml` |
| `rviz` | `true`，默认启动 RViz；无图形环境或一键脚本中可设置为 `false`。 |
| `rviz_config` | `$(find sunray_mavros_sim)/rviz/sunray_mavros_sim.rviz` |
...
| 参数 | 来源文件 | 说明 |
| --- | --- | --- |
| `uav/enable` | `sunray_mavros_sim_node.yaml` | 是否启动 UAV 仿真。 |
| `uav/agent_name` | `sunray_mavros_sim_node.yaml` | UAV 名称前缀，例如 `uav`。 |
| `uav/agent_ids` | `sunray_mavros_sim_node.yaml` | UAV 编号列表，例如 `[1]` 或 `[1, 2]`。 |
| `ugv/enable` | `sunray_mavros_sim_node.yaml` | 是否启动 UGV 仿真。 |
| `ugv/agent_name` | `sunray_mavros_sim_node.yaml` | UGV 名称前缀，例如 `ugv`。 |
| `ugv/agent_ids` | `sunray_mavros_sim_node.yaml` | UGV 编号列表，例如 `[1]` 或 `[1, 2]`。 |
| `agent_name`、`agent_ids` | `sunray_mavros_sim_node.yaml` | 兼容旧 UAV 配置的顶层参数，新配置优先使用 `uav/*`。 |
| `global_frame_id` | `sunray_mavros_sim_node.yaml` | 全包统一全局坐标系 frame，默认 `map`。全局地图、局部点云、TF、odom、navsat 都使用它。 |
| `enable_sensing` | `sunray_mavros_sim_node.yaml` | 是否创建全局地图和局部 MID360 模块。 |
...
| `global_map_topic` | `global_map_server.yaml` | 全局点云发布话题。默认 `/sunray_mavros_sim/global_cloud`，这是环境级公共话题，不带无人机前缀。 |
...
| 话题 | 类型 | 说明 |
| --- | --- | --- |
| `/sunray_mavros_sim/global_cloud` | `sensor_msgs/PointCloud2` | 全局 PCD 点云，话题名可由 `global_map_topic` 修改。 |
...
| 话题 | 类型 | 说明 |
| --- | --- | --- |
| `~/sunray_mavros_sim/odom` | `nav_msgs/Odometry` | 当前 UAV/UGV 位姿。 |
...
无。IMU 消息由 `QuadrotorSimulator` 或 `UgvSimulator` 发布到 `~/sunray_mavros_sim/imu`。
...
| 参数 | 来源文件 | 说明 |
| --- | --- | --- |
| `global_frame_id` | `sunray_mavros_sim_node.yaml` | odom 的 `header.frame_id`，默认 `map`。 |
...
| 话题 | 类型 | 说明 |
| --- | --- | --- |
| `~/sunray_mavros_sim/odom` | `nav_msgs/Odometry` | 仿真无人车里程计。 |
| `~/sunray_mavros_sim/imu` | `sensor_msgs/Imu` | 仿真 IMU。 |
...
| `~/sunray_mavros_sim/navsat` | `sensor_msgs/NavSatFix` | 内部仿真 GNSS。 |
...
| `visualizer/marker_topic` | `sim_visualizer.yaml` | 可视化话题。留空时使用 `~/sunray_mavros_sim/visualization`。 |
...
| 话题 | 类型 | 说明 |
| --- | --- | --- |
| `~/sunray_mavros_sim/odom` | `nav_msgs/Odometry` | UAV/UGV 位姿、速度和 frame。 |
| `~/sunray_mavros_sim/cmd_RPM` | `std_msgs/Float32MultiArray` | UAV 专用，用于状态面板显示平均电机转速。UGV 模式不依赖该话题。 |
| `~/mavros/state` | `mavros_msgs/State` | UAV 专用，用于状态面板显示 fake MAVROS 是否正常。UGV 模式不依赖该话题。 |
| `~/sunray_mavros_sim/cloud_world_frame` | `sensor_msgs/PointCloud2` | 用于状态面板显示最近局部点云点数。 |

发布话题：

| 话题 | 类型 | 说明 |
| --- | --- | --- |
| `~/sunray_mavros_sim/visualization` | `visualization_msgs/MarkerArray` | 单一 RViz 可视化话题，包含 UAV/UGV 模型、轨迹和文字。 |
...
rostopic hz ~/sunray_mavros_sim/visualization
...
rostopic hz ~/sunray_mavros_sim/odom
rostopic echo ~/sunray_mavros_sim/odom -n 1
```

启动时默认会打开 RViz。如果不需要图形界面：

```bash
roslaunch sunray_mavros_sim sunray_mavros_sim.launch rviz:=false
```

运行最小闭环验证脚本：

```bash
rosrun sunray_mavros_sim minimal_closed_loop_check.py
```

该脚本会自动启动 `sunray_mavros_sim.launch`，并临时生成一份单机 `agent_ids: [1]` 和 `vehicles/uav1/init_*` 配置，持续发布一个 `z=1.0 m` 的 MAVROS `PositionTarget`，然后检查：

| 检查项 | 默认要求 |
| --- | --- |
| `~/sunray_mavros_sim/odom` | 收到 odom，且 z 有明显上升。 |
| `~/sunray_mavros_sim/imu` | 收到 IMU。 |
| `~/mavros/state` | 收到 fake MAVROS 状态。 |
| `~/sunray_mavros_sim/cloud_world_frame` | 收到局部点云。 |
| `~/sunray_mavros_sim/visualization` | 收到 RViz MarkerArray。 |
| `~/sunray_mavros_sim/cmd_RPM` | 收到有效电机 RPM 输出。 |

如果已经手动启动了仿真节点，只想跑检查：

```bash
rosrun sunray_mavros_sim minimal_closed_loop_check.py --no-launch
```

检查集群中的第二架无人机时，先按双机配置手动启动仿真，再运行：

```bash
rosrun sunray_mavros_sim minimal_closed_loop_check.py --no-launch --agent-id 2
```
