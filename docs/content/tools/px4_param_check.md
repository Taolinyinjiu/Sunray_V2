<!-- title: px4_param_check -->

<section id="tools-px4-param-check">

## px4_param_check

`tools/px4_param_check` 通过 MAVROS 参数服务检查或写入 Sunray 关心的 PX4 参数，适合真机起飞前确认机型参数是否符合 Sunray 要求。
...
```text
tools/px4_param_check/config/px4_params_default.yaml
tools/px4_param_check/config/px4_params_s150.yaml
tools/px4_param_check/config/px4_params_s300.yaml
```
...
可按机型或任务新增配置，例如 `px4_params_s150.yaml`、`px4_params_s300.yaml`。配置中的 `parameter_group` 用来分组管理参数，`--group` 需要填写完整分组名。

### 检查 PX4 参数

```bash
cd tools/px4_param_check
...
- 新机型参数模板应新增 `config/px4_params_<airframe>.yaml`。
- 参数说明要写清楚推荐值、单位、适用机型和风险。
- 不建议任务节点运行时频繁改 PX4 参数。
- 如果新增必须检查的 PX4 参数，应同步更新机型配置、真机部署文档和起飞前检查流程。
