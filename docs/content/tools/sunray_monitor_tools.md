<!-- title: sunray_monitor_tools -->

<section id="tools-sunray-monitor-tools">
...
速度类单机指令会以 5 Hz 持续发布，点击 `停止速度` 后停止。位置、起降、HOLD 等事件类命令通常只发布一次。
...
- 新增飞机/车：复制 agent 配置并修改 `id`、`namespace` 和话题。
- 新增状态字段：同步更新对应 `sunray_msgs`、状态发布者和面板解析逻辑。
- 新增控制按钮：先确认底层控制消息是否已经有对应命令，再同步 UI、命令发布逻辑和安全提示。
- 速度类单机指令由面板持续发布；位置、起降类指令只发一次。
