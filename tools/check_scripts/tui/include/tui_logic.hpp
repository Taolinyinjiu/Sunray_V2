...
private:
    ...
    static constexpr const char* LAST_SELECTION_FILE = "tools/check_scripts/tui/build/.sunray_last_check_selection";
    ...
    /**
     * @brief 在检查模式下运行命令并捕获输出到TUI日志面板
     */
    int run_command_in_check_panel(const std::string& title,
    ...
    /**
     * @brief 将一行输出追加到检查日志面板
     */
    void append_check_log_line(const std::string& line);
};
