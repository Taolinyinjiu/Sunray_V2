...
struct UIState {
   ...
   * @brief 检查模式下的执行过程输出
   *
   * Check TUI 不再在点击按钮后退出到 CLI，而是在界面底部展示检查或解决
   * 依赖过程。Build 模式不会使用这些字段。
   */
  std::vector<std::string> check_log_lines;
  std::string check_task_title = "执行过程";
  bool check_task_running = false;
  int check_task_exit_code = 0;
