...
ftxui::Element UIRenderer::render_check_output_panel(int content_width) {
  constexpr int panel_content_height = 7;
  std::vector<Element> rows;

  std::string title = state_.check_task_title.empty() ? "执行过程"
                                                       : state_.check_task_title;
  if (state_.check_task_running) {
    title += " (运行中)";
  } else if (!state_.check_log_lines.empty()) {
    title += state_.check_task_exit_code == 0 ? " (完成)" : " (失败)";
  }

  rows.push_back(text(title) | bold |
                 color(state_.check_task_exit_code == 0 ? Color::Cyan
                                                        : Color::Red));
  ...
  if (state_.check_log_lines.empty()) {
    ...
    const int total = static_cast<int>(state_.check_log_lines.size());
    const int begin = std::max(0, total - max_log_rows);
    for (int i = begin; i < total; ++i) {
      std::string line = state_.check_log_lines[i];
      if (line.empty()) {
        rows.push_back(text(" "));
        continue;
      }
      rows.push_back(paragraph(line) |
                     size(WIDTH, EQUAL, std::max(20, content_width - 4)));
    }
  }

  while (static_cast<int>(rows.size()) < panel_content_height) {
    rows.push_back(text(" "));
  }

  return vbox(rows) | border | size(HEIGHT, EQUAL, panel_content_height + 2);
}
...
ftxui::Component UIRenderer::create_component() {
  ...
  Component final_renderer = Renderer(event_handler, [this] {
    ...
    const int check_output_height = state_.app_mode == AppMode::Check ? 10 : 0;
    const int fixed_ui_height =
        1 + 1 + 1 + (1 + extra_desc_lines) + 3 + 1 + 1 +
        check_output_height + key_guide_height + debug_height;
    ...
    if (state_.app_mode == AppMode::Check) {
      main_content.push_back(separatorLight());
      main_content.push_back(render_check_output_panel(content_width));
    }
