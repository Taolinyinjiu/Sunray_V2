...
find_check_config_file() {
    ...
    if [[ -n "${CHECKSCRIPTS_DIR:-}" && -f "${CHECKSCRIPTS_DIR}/module_config.yaml" ]]; then
        echo "${CHECKSCRIPTS_DIR}/module_config.yaml"
        return 0
    fi

    if [[ -n "${SCRIPT_DIR:-}" && -f "${SCRIPT_DIR}/tools/check_scripts/module_config.yaml" ]]; then
        echo "${SCRIPT_DIR}/tools/check_scripts/module_config.yaml"
        return 0
    fi

    if [[ -f "tools/check_scripts/module_config.yaml" ]]; then
        echo "tools/check_scripts/module_config.yaml"
        return 0
    fi
