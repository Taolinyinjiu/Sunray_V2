...
load_modules_config() {
    ...
    if [[ -n "${CHECKSCRIPTS_DIR:-}" && -f "${CHECKSCRIPTS_DIR}/modules.yaml" ]]; then
        config_file="${CHECKSCRIPTS_DIR}/modules.yaml"
    elif [[ -f "${SCRIPT_DIR}/tools/check_scripts/modules.yaml" ]]; then
        config_file="${SCRIPT_DIR}/tools/check_scripts/modules.yaml"
    else
...
resolve_mutex_state_file() {
    ...
    local runtime_dir=""
    if [[ -n "${CHECKSCRIPTS_DIR:-}" ]]; then
        runtime_dir="${CHECKSCRIPTS_DIR}/tui/build"
    elif [[ -n "${SCRIPT_DIR:-}" ]]; then
        runtime_dir="${SCRIPT_DIR}/tools/check_scripts/tui/build"
    else
        runtime_dir="tools/check_scripts/tui/build"
    fi
