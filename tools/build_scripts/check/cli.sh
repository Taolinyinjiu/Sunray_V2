...
CHECK_BUILDSCRIPTS_ROOT="${BUILDSCRIPTS_DIR:-tools/build_scripts}"
CHECK_LAST_SELECTION_FILE="${CHECK_BUILDSCRIPTS_ROOT}/tui/build/.sunray_last_build_selection"

get_check_modules_config_file() {
    if [[ -n "${BUILDSCRIPTS_DIR:-}" && -f "${BUILDSCRIPTS_DIR}/modules.yaml" ]]; then
        echo "${BUILDSCRIPTS_DIR}/modules.yaml"
    elif [[ -n "${SCRIPT_DIR:-}" && -f "${SCRIPT_DIR}/tools/build_scripts/modules.yaml" ]]; then
        echo "${SCRIPT_DIR}/tools/build_scripts/modules.yaml"
    elif [[ -f "tools/build_scripts/modules.yaml" ]]; then
        echo "tools/build_scripts/modules.yaml"
...
${YELLOW}说明:${NC}
  ...
  • 检查依赖不会安装软件，也不会编译模块
  • 解决依赖会执行 tools/build_scripts/check/reslove/<module>/*.sh
