...
void UIState::update_module_render_items() {
  ...
  // 2. 按 label 分组；label 默认来自 modules.yaml，缺省时退回到 source_path 第一段。
  ...
  std::stable_sort(
      ...
      [&](const Module *lhs, const Module *rhs) {
        std::string lhs_label = lhs->label.empty() ? "other" : lhs->label;
        std::string rhs_label = rhs->label.empty() ? "other" : rhs->label;
  ...
  for (const auto &[label, modules] : modules_by_label) {
