...
// 规范化输入的agent_key,目标agent_key = /agent_name + agent_id
...
// 将输入的字符串中的"${agent_key}"转译为输入的agent_key
inline std::string replace_agent_key(std::string input_string, const std::string& agent_key) {
    // 首先拉出字符串常量，"${agent_key}”占位符
    const std::string agent_string = "${agent_key}";
    // 如果存在agent_string的话，他在原字符串中的位置
    std::size_t pos = 0;
    // 首先判断两者都非空串
    ...
    // 是否有占位符
    if (input_string.find(agent_string) == std::string::npos) {
        // 没有发现占位符的话，返回原字符串
        return input_string;
    }
    // 存在占位符，进行转译
    // input_string.fing(agent_string,pos)
    // 表示从pos位置开始查找，agent_string字串，如果查找到,则返回查找到的位置，如果没有查找到，会返回std::string::npos
    // 如果查找到一次，就使用replace方法，将
    while ((pos = input_string.find(agent_string, pos)) != std::string::npos) {
        // 从input.string的pos位置开始，将长度为uav_string字符串长度的内容，替换为uav_ns
        input_string.replace(pos, agent_string.length(), agent_key);
...
// 下面这个函数似乎没有被使用，也许是设计过程中的草稿
inline std::string replace_string(std::string raw_string,
                                  const std::string& occupy_string,
