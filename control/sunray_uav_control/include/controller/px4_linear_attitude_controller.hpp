...
#include "core_algorithm/linear_attitude_control.hpp"

class PX4_LinearAttitude_Controller : public Controller_Interface {
  public:
    PX4_LinearAttitude_Controller(ros::NodeHandle& nh);
    ~PX4_LinearAttitude_Controller() = default;
    // ------------生命周期相关----------------
  ...
  private:
    ...
    std::string uav_ns_;
    Linear_AttitudeControl_Param_t linear_controller_param_;
    Linear_AttitudeControl controller;
    int fuse_odom_type;
    double fuse_odom_frequency{0.0};
    double motors_speedup_time_{3.0};
    // ----------------------检查相关-----------------------
    ...
    // ---------------------起飞辅助函数--------------------
    controller_data_types::TargetTrajectoryPoint_t get_rotor_speed_up_des(const ros::Time now);
    // -------------------ros句柄与mavros辅助类-----------------
    ros::NodeHandle nh_;
