...
class Sunray_FSM {
  public:
    ...
    void process();  // 状态机更新，将数据丢进控制器，或者根据当前state与controlcmd决定输出
  ...
  private:
    ...
    // 放弃上面的思路，只要进入Sunray状态机的流程，就应该是OFFBOARD模式，position模式手飞玩玩得了
    bool takeoff();
    bool land();
    ...
    std::queue<sunray_fsm::SunrayEvent> fsm_event_list;                 // 事件队列
