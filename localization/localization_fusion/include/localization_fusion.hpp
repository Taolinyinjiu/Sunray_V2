...
private:
  ...
  // ---- 发布相关----
	/*
	发布的逻辑是这样定义的
	1. 对于local和global模式，他们的输入源只有一个单独的odometry,这个模式下sunray_local和sunray_global之间的tf关系为原点重合，姿态单位阵
	因此直接从odometry_callback中，将输入的里程计定义为sunray_local的，并通过默认的tf转换为sunray_global的
	2. 对于aruco这样的，间断的重定位，其重定位数据从relocalization_callback重定位话题的回调得到，
	由于这样的重定位次数不定，频率不定，因此使用回调数据修改tf树，然后在定时器中发布动态的tf而不是在回调函数中发布
	3. 对于fastlio2_relocalization这样的重定位模式，他实际上会有local的数据和global的数据，这里global的数据指的是重定位算法，在完成重定位后回输出当前的位姿在先验地图中的位姿，
	...
	4. 由于我们使用了10Hz的定时器进行状态检查，因此将OdomStatus的发布也放置在healthtimer_callback中
	*/
