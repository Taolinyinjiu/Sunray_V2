...
inline std::string load_uav_namespace_or_throw(ros::NodeHandle& nh) {
    ...
    } else if (private_nh.getParam("ugv_name", uav_name)) {
        if (uav_name.empty()) {
    ...
    } else if (nh.getParam("/ugv_name", uav_name)) {
        if (uav_name.empty()) {
            throw std::runtime_error("/ugv_name cannot be empty");
        }
    } else {
        throw std::runtime_error("missing param ~uav_name/~ugv_name or /uav_name//ugv_name");
    }
    ...
    } else if (private_nh.getParam("ugv_id", uav_id)) {
        if (uav_id <= 0) {
    ...
    } else if (nh.getParam("/ugv_id", uav_id)) {
        if (uav_id <= 0) {
            throw std::runtime_error("/ugv_id cannot <= 0");
        }
    } else {
        throw std::runtime_error("missing param ~uav_id/~ugv_id or /uav_id//ugv_id");
    }
