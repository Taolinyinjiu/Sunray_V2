...
detect_vins_root() {
  ...
  if [[ -d "$base/localization/third_party_localization/vins-fusion-gpu/loop_fusion" && \
        -d "$base/localization/third_party_localization/vins-fusion-gpu/vins_estimator" && \
        -d "$base/localization/third_party_localization/vins-fusion-gpu/camera_models" ]]; then
    printf '%s\n' "$base/localization/third_party_localization/vins-fusion-gpu"
    return 0
  fi
...
if vins_root="$(detect_vins_root "$PWD")"; then
  ...
  echo "Run this script from the repository root or from localization/third_party_localization/vins-fusion-gpu."
  exit 1
fi

for cmd in find grep perl; do
