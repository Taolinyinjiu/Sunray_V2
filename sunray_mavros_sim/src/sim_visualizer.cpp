      ...
      agent_name_(agent_name)
{
    ...
    nh_.param<std::string>("visualizer/marker_topic",
                           marker_topic_,
                           agent_prefix_ + "/sunray_mavros_sim/visualization");
    if (marker_topic_.empty())
    {
        marker_topic_ = agent_prefix_ + "/sunray_mavros_sim/visualization";
    }

    odom_sub_ = nh_.subscribe<nav_msgs::Odometry>(
        agent_prefix_ + "/sunray_mavros_sim/odom", 20, &SimVisualizer::odomCallback, this);
    rpm_sub_ = nh_.subscribe<std_msgs::Float32MultiArray>(
        agent_prefix_ + "/sunray_mavros_sim/cmd_RPM", 20, &SimVisualizer::rpmCallback, this);
    ...
    cloud_sub_ = nh_.subscribe<sensor_msgs::PointCloud2>(
        agent_prefix_ + "/sunray_mavros_sim/cloud_world_frame", 5, &SimVisualizer::cloudCallback, this);
    ...
    ROS_INFO("[sunray_mavros_sim] visualizer started for %s, marker_topic=%s",
             agent_prefix_.c_str(),
             marker_topic_.c_str());
}
...
void SimVisualizer::printStatus() const
{
    ...
    ss << kAnsiGood << " 订阅话题 " << kAnsiReset
       << "odom -> " << agent_prefix_ << "/sunray_mavros_sim/odom";
    if (!is_ugv_)
    {
        ss << "  cmd_RPM -> " << agent_prefix_ << "/sunray_mavros_sim/cmd_RPM"
           << "\n"
           << "          mavros_state -> " << agent_prefix_ << "/mavros/state";
    }
    ss << "  local_cloud -> " << agent_prefix_ << "/sunray_mavros_sim/cloud_world_frame"
       << "\n";
