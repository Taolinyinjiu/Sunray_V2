    ...
    : nh_(nh), agent_prefix_("/" + agent_name + std::to_string(std::max(agent_id, 1))), global_map_(std::move(global_map))
{
    ...
    odom_sub_ = nh_.subscribe<nav_msgs::Odometry>(agent_prefix_ + "/sunray_mavros_sim/odom",
                                                  50,
    ...
    ROS_INFO("[sunray_mavros_sim] local %s simulator started for %s, map_points=%zu",
             lidar_type_.c_str(),
             agent_prefix_.c_str(),
