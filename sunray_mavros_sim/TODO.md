# sunray_mavros_sim 轻量仿真 TODO

本文档用于整理 `sunray_mavros_sim` 后续修改方向。目标不是复刻 Gazebo 的全部物理和传感器细节，而是做一个更轻、更容易启动、更适合规划/控制闭环调试的 MAVROS 风格仿真器。
...
- [ ] 第三方规划器旧入口命名：`planning/third_party_planner_examples/*/run_pengyusim_lio.launch` 仍保留历史文件名和注释。本轮没有修改这些入口，避免影响外部规划器兼容性；后续可确认是否新增/改名为 `run_sunray_mavros_sim_lio.launch`。
...
- 当前状态：
  - `sunray_mavros_sim_node.cpp`：唯一入口，负责读取通用参数、创建地图，并按 `agent_ids` 创建一架或多架无人机仿真。
