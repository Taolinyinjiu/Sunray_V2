...
class TopicMonitor:
    def __init__(self, agent_prefix, check_sensing):
        ...
        rospy.Subscriber(agent_prefix + "/sunray_mavros_sim/odom", Odometry, self.odom_cb, queue_size=50)
        rospy.Subscriber(agent_prefix + "/sunray_mavros_sim/imu", Imu, self.imu_cb, queue_size=50)
        rospy.Subscriber(agent_prefix + "/mavros/state", State, self.state_cb, queue_size=50)
        rospy.Subscriber(agent_prefix + "/sunray_mavros_sim/cmd_RPM", Float32MultiArray, self.cmd_rpm_cb, queue_size=50)
        rospy.Subscriber(agent_prefix + "/sunray_mavros_sim/visualization", MarkerArray, self.visualization_cb, queue_size=10)
        if check_sensing:
            rospy.Subscriber(agent_prefix + "/sunray_mavros_sim/cloud_world_frame", PointCloud2, self.cloud_cb, queue_size=10)
...
def create_node_config(args):
    config_file = tempfile.NamedTemporaryFile(
        mode="w",
        prefix="sunray_mavros_sim_test_node_",
        suffix=".yaml",
        delete=False,
    )
...
def create_single_uav_config(args):
    ...
    config_file = tempfile.NamedTemporaryFile(
        mode="w",
        prefix="sunray_mavros_sim_test_vehicle_",
        suffix=".yaml",
        delete=False,
    )
...
def start_launch(args):
    package_path = rospkg.RosPack().get_path("sunray_mavros_sim")
    launch_file = os.path.join(package_path, "launch", "sunray_mavros_sim.launch")
...
def evaluate(monitor, args, start_z):
    ...
    print("")
    print("================ sunray_mavros_sim 最小闭环验证结果 ================")
...
def parse_args():
    parser = argparse.ArgumentParser(description="sunray_mavros_sim 最小闭环验证脚本")
...
def main():
    ...
    try:
        if not args.no_launch:
            print("[sunray_mavros_sim test] 启动仿真 launch...")
            launch_parent, temp_node_config, temp_single_uav_config = start_launch(args)
            time.sleep(1.0)

        rospy.init_node("sunray_mavros_sim_minimal_closed_loop_check", anonymous=True)
        ...
        print("[sunray_mavros_sim test] 等待 odom...")
        if not wait_for_odom(monitor, args.startup_timeout):
            print("[sunray_mavros_sim test] 失败：启动后没有收到 odom")
            return 1
        ...
        print("[sunray_mavros_sim test] 发布悬停 setpoint: z=%.2f m, topic=%s" % (
            args.target_z,
        ...
        if launch_parent is not None:
            print("[sunray_mavros_sim test] 关闭仿真 launch...")
