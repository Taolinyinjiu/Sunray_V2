...
int main(int argc, char** argv) {
    ...
    // 清理循环变量
    times = 0;
    // 圆形轨迹中心需要使用当前里程计，进入任务前先等待有效 local_odom
    while (ros::ok() && (!has_odom || !is_odom_valid(uav_odom))) {
        ROS_WARN_THROTTLE(1.0, "waiting for valid local odom before circle trajectory task...");
        ros::spinOnce();
        ros::Duration(0.1).sleep();
    }
    // 完成起飞后，从当前位置生成圆轨迹
    ROS_INFO("uav takeoff succeeded, tracking circle trajectory");
    ...
    while (ros::ok()) {
        ...
        if (!circle_finished) {
            ...
            publish_trajectory_command(make_hold_reference(start_point));
            if (check_arrived(start_point, uav_odom)) {
                ROS_INFO("circle trajectory example finished, now sending RETURN");
                break;
            }
        }
        ros::spinOnce();
    ...
    // 直接结束节点或等待成功降落？
    ROS_INFO("sent RETURN command and [circle_trajectory] demo finished, quit!");
    return 0;
}
