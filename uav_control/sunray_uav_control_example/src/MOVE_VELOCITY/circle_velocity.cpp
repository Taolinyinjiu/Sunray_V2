...
int main(int argc, char** argv) {
    ...
    while (ros::ok() && (!has_odom || !is_odom_valid(uav_odom))) {
        ROS_WARN_THROTTLE(1.0, "waiting for valid local odom before circle velocity task...");
        ros::spinOnce();
        ros::Duration(0.1).sleep();
    }
    // 完成起飞后，使用 MOVE_VELOCITY 先移动到圆轨迹起点
    ...
    while (ros::ok()) {
        ...
        if (!circle_finished) {
            ...
            if (check_arrived(start_point, uav_odom)) {
                ROS_INFO("circle example finished, now sending RETURN");
                break;
            }
        }
        ros::spinOnce();
    ...
    // 直接结束节点或等待成功降落？
    ROS_INFO("sent RETURN command and [circle_velocity] demo finished, quit!");
    return 0;
}
