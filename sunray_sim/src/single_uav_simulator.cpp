      ...
      enable_sensing_(enable_sensing)
{
    ...
    quadrotor_.reset(new QuadrotorSimulator(nh_, agent_name, agent_id, init_pos_, init_yaw_));
    px4_control_.reset(new Px4ControlSim(nh_, agent_prefix_));
    fake_mavros_.reset(new FakeMavrosBridge(nh_, agent_prefix_));
...
void SingleUavSimulator::printStatus() const
{
    ...
    px4_control_->printStatus();
    fake_mavros_->printStatus();
}
}  // namespace sunray_sim
