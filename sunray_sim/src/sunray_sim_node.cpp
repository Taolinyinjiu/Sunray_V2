...
std::vector<int> loadAgentIds(ros::NodeHandle& nh, const std::string& param_name)
{
    ...
    if (nh.getParam(param_name, agent_ids_param))
    {
        ...
        {
            ROS_ERROR("[sunray_sim] %s must be a YAML list, for example: %s: [1, 2]",
                      param_name.c_str(),
                      param_name.c_str());
            return agent_ids;
        }
        ...
        for (int i = 0; i < agent_ids_param.size(); ++i)
        {
            ...
            {
                ROS_WARN("[sunray_sim] ignore non-integer %s[%d]", param_name.c_str(), i);
                continue;
            }
    ...
    if (agent_ids.empty())
    {
        ROS_ERROR("[sunray_sim] %s is empty, please set at least one vehicle id", param_name.c_str());
        return agent_ids;
    }
...
{
    // 这个进程是 sunray_sim 的唯一运行入口。所有功能模块都在同一个
    // ROS node 内创建，launch 时不再分别启动地图、局部雷达、动力学、PX4
    // 控制转换和 fake MAVROS 多个 node。
    ros::init(argc, argv, "sunray_sim_node");
    ros::NodeHandle nh("~");
    ...
    {
        ROS_ERROR("[sunray_sim] both uav/enable and ugv/enable are false");
        return 1;
    }
    ...
    std::unique_ptr<sunray_sim::GlobalMapServer> map_server;
    pcl::PointCloud<pcl::PointXYZI>::ConstPtr global_map;
    if (enable_sensing)
    {
        map_server.reset(new sunray_sim::GlobalMapServer(nh));
        if (!map_server->ready())
        {
            ROS_ERROR("[sunray_sim] failed to initialize global map, node will exit");
            return 1;
        }
    ...
    std::vector<std::unique_ptr<sunray_sim::SingleUavSimulator>> uav_simulators;
    std::vector<std::unique_ptr<sunray_sim::SingleUgvSimulator>> ugv_simulators;
    std::vector<std::unique_ptr<sunray_sim::SimVisualizer>> visualizers;
    ...
    {
        uav_simulators.emplace_back(new sunray_sim::SingleUavSimulator(nh,
                                                                              global_map,
                                                                              enable_sensing,
                                                                              uav_agent_name,
                                                                              agent_id));
        ...
        {
            visualizers.emplace_back(new sunray_sim::SimVisualizer(nh, uav_agent_name, agent_id));
        }
    }
    ...
    ROS_INFO("[sunray_sim] integrated node started for uav=[%s], ugv=[%s], global_frame_id=%s",
             uav_agent_prefixes.c_str(),
             ugv_agent_prefixes.c_str(),
             global_frame_id.c_str());
    if (enable_sensing)
    {
        ROS_INFO("[sunray_sim] sensing outputs: %s, /<agent>/sunray_sim/cloud_world_frame, /<agent>/sunray_sim/cloud_sensor_frame",
                 map_server->topic().c_str());
    }
    else
    {
        ROS_INFO("[sunray_sim] sensing disabled, global map and local MID360 are not started");
    }
    ROS_INFO("[sunray_sim] dynamics outputs: /<agent>/sunray_sim/odom, /<agent>/sunray_sim/imu");
    ROS_INFO("[sunray_sim] MAVROS facade: /<uav>/mavros/*");
    if (!visualizers.empty())
    {
        ROS_INFO("[sunray_sim] visualization output: /<agent>/sunray_sim/visualization");
    }
        ...
        status_timer = nh.createTimer(
            ...
            [&](const ros::TimerEvent&) {
                std::cout << "\033[1;44;37m"
                          << "=================== sunray_sim [uav=" << uav_simulators.size()
