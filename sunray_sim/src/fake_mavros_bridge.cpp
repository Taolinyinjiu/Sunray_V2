...
FakeMavrosBridge::FakeMavrosBridge(ros::NodeHandle& nh, const std::string& uav_name)
    ...
    odom_sub_ = nh_.subscribe(uav_name_ + "/sunray_sim/odom",
                              20,
                              &FakeMavrosBridge::odomCallback,
                              this);
    imu_sub_ = nh_.subscribe(uav_name_ + "/sunray_sim/imu",
                             20,
                             &FakeMavrosBridge::imuCallback,
                             this);
    navsat_sub_ = nh_.subscribe(uav_name_ + "/sunray_sim/navsat",
                                20,
...
std::string FakeMavrosBridge::buildStatusPanel() const {
    ...
    ss << kAnsiGood << " 订阅话题 " << kAnsiReset
       << "里程计 -> " << uav_name_ << "/sunray_sim/odom"
       << "  |  IMU -> " << uav_name_ << "/sunray_sim/imu"
       << "  |  GNSS -> " << uav_name_ << "/sunray_sim/navsat"
       << "\n"
