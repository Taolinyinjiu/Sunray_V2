...
{
    const std::string package_path = ros::package::getPath("sunray_sim");
...
bool GlobalMapServer::loadMap()
{
    if (map_name_.empty())
    {
        ROS_ERROR("[sunray_sim] map_name is empty");
        return false;
    }
    ...
    {
        ROS_ERROR("[sunray_sim] cannot read PCD map: %s", map_name_.c_str());
        return false;
    }
    ...
    ROS_INFO("[sunray_sim] loaded map %s, points=%zu, bound x=(%.2f, %.2f), y=(%.2f, %.2f), z=(%.2f, %.2f)",
             map_name_.c_str(),
             cloud_->size(),
             min_pt.x,
             max_pt.x,
