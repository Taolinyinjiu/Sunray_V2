/*
本程序功能：
    1、订阅 UGVSwarmState，缓存每辆无人车的集群状态
    2、发布 RViz MarkerArray，显示无人车 mesh、速度、ID、轨迹、目标点和当前任务
*/
#include <algorithm>
#include <cmath>
#include <deque>
...
class RvizVisualizationUgvNode
{
  public:
    RvizVisualizationUgvNode()
        : nh_(), private_nh_("~")
    {
        ...
        private_nh_.param("mesh_resource", mesh_resource_,
                          std::string("package://sunray_swarm_control/utils/meshes/ugv.dae"));
        ...
        swarm_state_sub_ =
            nh_.subscribe(swarm_state_topic_, 100, &RvizVisualizationUgvNode::swarmStateCallback, this);
        ...
        publish_timer_ = nh_.createTimer(ros::Duration(1.0 / publish_hz_),
                                         &RvizVisualizationUgvNode::publishTimerCallback, this);

        ROS_INFO("rviz_visualization_ugv_node ready: state_topic=%s marker_topic=%s mesh=%s",
  ...
  private:
    ...
    void appendWorldOriginMarkers(const ros::Time &stamp, visualization_msgs::MarkerArray &marker_array) const
    {
        ...
        visualization_msgs::Marker sphere = baseOriginMarker(0, stamp);
        sphere.type = visualization_msgs::Marker::SPHERE;
        sphere.pose.position = origin;
        sphere.scale.x = 0.16;
        sphere.scale.y = 0.16;
        sphere.scale.z = 0.16;
        sphere.color = makeColor(1.0, 1.0, 1.0, 1.0);
        marker_array.markers.push_back(sphere);

        appendAxisMarker(1, stamp, origin, 1.0, 0.0, 0.0, makeColor(1.0, 0.1, 0.1, 1.0), marker_array);
        appendAxisMarker(2, stamp, origin, 0.0, 1.0, 0.0, makeColor(0.1, 1.0, 0.1, 1.0), marker_array);

        visualization_msgs::Marker text = baseOriginMarker(3, stamp);
        text.type = visualization_msgs::Marker::TEXT_VIEW_FACING;
        text.pose.position.z = 0.35;
        text.scale.z = text_height_;
        text.color = makeColor(1.0, 1.0, 1.0, 1.0);
        text.text = "world origin";
    ...
    void appendStaticObstacleMarkers(const ros::Time &stamp, visualization_msgs::MarkerArray &marker_array) const
    {
        for (size_t i = 0; i < static_obstacle_x_.size(); ++i)
        {
            ...
            text.pose.position.z = static_obstacle_height_ + 0.30;
            text.scale.z = text_height_ * 0.75;
    ...
    void appendAgentMarkers(const int agent_id,
                            ...
                            visualization_msgs::MarkerArray &marker_array) const
    {
        const sunray_msgs::UGVSwarmState &state = agent.state;
        ...
        task_text.pose.position.z += 0.75;
    ...
    void appendTargetMarkers(const int agent_id,
                             const sunray_msgs::UGVSwarmState &state,
                             const ros::Time &stamp,
                             visualization_msgs::MarkerArray &marker_array) const
    {
    ...
    static std::string taskText(const sunray_msgs::UGVSwarmState &state)
    {
        std::ostringstream ss;
...
{
    ros::init(argc, argv, "rviz_visualization_ugv_node");
    RvizVisualizationUgvNode node;
    ros::spin();
    return 0;
}
