...
// 全局固定高度，MOVE_VELOCITY 接口通过 fixed_height 字段表达 z 轴目标
double fixed_height{0.6};

// 多个目标点,这里是矩形的四个角点
Eigen::Vector3d Point_1(1, 1, fixed_height);
Eigen::Vector3d Point_2(1, -1, fixed_height);
Eigen::Vector3d Point_3(-1, -1, fixed_height);
Eigen::Vector3d Point_4(-1, 1, fixed_height);
// 目标点容器
...
void velocity_controller_update(Eigen::Vector3d desired_pos, const nav_msgs::Odometry& odom) {
    ...
    // MOVE_VELOCITY 采用“xy速度 + fixed_height”的混合语义，z 轴不再直接输出速度。
    velocity_cmd.z() = 0.0;
    // 对输出进行限制幅度，仅限制 xy 两个方向速度
    for (int i = 0; i < 2; ++i) {
