...
LemniscateReference sample_lemniscate_reference(double phase,
    ...
    // 将双纽线旋转为沿y轴展开，并尽量让x/y参数接近对应方向的峰峰值
    // sin*cos/(1+sin^2) 的峰峰值为 1/sqrt(2)，因此乘以 sqrt(2) 做归一化
    // cos/(1+sin^2) 的峰峰值为 2，因此乘以 0.5 做归一化
    ...
    reference.desired_pos = lemniscate_center;
    reference.desired_pos.x() += x_rel;
    reference.desired_pos.y() += y_rel;

    const double tangent_norm = std::hypot(dx_dphase, dy_dphase);
    if (tangent_norm > 1e-6) {
        reference.phase_rate = kLemniscateSpeed / tangent_norm;
    } else {
        reference.phase_rate = 0.0;
    }

    reference.desired_vel_ff.x() = dx_dphase * reference.phase_rate;
    reference.desired_vel_ff.y() = dy_dphase * reference.phase_rate;
...
void mySignalHandler(int sig) {
    std::cout << "sunray_uav_example [lemniscate_velocity] node exit..." << std::endl;
    ros::shutdown();
...
int main(int argc, char** argv) {
    // ros节点初始化
    ros::init(argc, argv, "lemniscate_velocity_node");
