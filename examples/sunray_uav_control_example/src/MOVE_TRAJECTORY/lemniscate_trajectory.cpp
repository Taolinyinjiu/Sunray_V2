...
int main(int argc, char** argv) {
    ...
    // 清理循环变量
    times = 0;
    // 8字轨迹中心需要使用当前里程计，进入任务前先等待有效 local_odom
    while (ros::ok() && (!has_odom || !is_odom_valid(uav_odom))) {
        ROS_WARN_THROTTLE(1.0, "waiting for valid local odom before lemniscate trajectory task...");
        ros::spinOnce();
        ros::Duration(0.1).sleep();
    }
    // 完成起飞后，直接从当前位置作为双纽线中心开始轨迹追踪
    ROS_INFO("uav takeoff succeeded, tracking lemniscate trajectory");
    ...
    while (ros::ok()) {
        if (!lemniscate_finished) {
            ...
            if (check_arrived(center_point, uav_odom)) {
                ROS_INFO("lemniscate trajectory example finished, now sending RETURN");
                break;
            }
        }
        ros::spinOnce();
    ...
    // 直接结束节点或等待成功降落？
    ROS_INFO("sent RETURN command and [lemniscate_trajectory] demo finished, quit!");
    return 0;
}
