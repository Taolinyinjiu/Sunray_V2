...
int main(int argc, char** argv) {
    ...
    ROS_INFO(
        "uav takeoff successfully and now move to defined points");
    ...
    for (int i = 0; i < Expect_Points.size(); i++) {
        ...
        while (ros::ok() &&
            ...
            // 发布降落命令
            if (times++ > 5) {
                ROS_ERROR("Error : sunray control module can't checkout move state,please check "
                          "sunray log");
                uav_cmd.header.stamp = ros::Time::now();
    ...
    // 直接结束节点或等待成功降落？
    ROS_INFO("uav is enter land mode and [move_multipoint] demo finished,quit !");
    return 0;
}
