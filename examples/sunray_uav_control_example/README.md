# sunray_uav_control_example 控制例程说明

这个包集中演示如何通过 `sunray_msgs/UAVControlCMD` 调用 `sunray_uav_control` 的上层控制接口。相比 `control/sunray_uav_control/README.md`，本文更关注每个示例程序怎么启动、会做什么、代码里该改哪里，以及新手最容易踩到的点。
...
示例节点只发 Sunray 控制命令，不能单独完成飞控控制。运行任意例程前，至少需要先启动：
...
Sunray 示例节点通常不直接控制电机，而是通过 `control_cmd` 触发 `sunray_uav_control` 的状态机。新手可以先记住下面几类流程。
...
时序上最容易出错的是最后一类：`TAKEOFF`、`LAND`、`RETURN`、`MOVE_POINT` 是事件型命令，通常发一次；`MOVE_VELOCITY` 和 `MOVE_TRAJECTORY` 是连续参考量，任务运行期间必须按固定频率持续发。
...
### 9.2 move_velocity_fixed_height
...
## 10. MOVE_VELOCITY_BODY 示例

### 10.1 move_velocity_body
