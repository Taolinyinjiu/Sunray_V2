...
// 无人机当前状态回调函数
void Sunray_FSM_State_Callback(const sunray_msgs::UAVControlFSMState& msg) {
...
int main(int argc, char** argv) {
    ...
    ROS_INFO("sunray_uav wikk takeoff and hover 10s then land");
    ...
    while (ros::ok() && !system_ready) {
        ...
        if (has_uav_name && !uav_name.empty() && has_uav_id) {
            system_ready = true;
            uav_namespace = uav_name + std::to_string(uav_id);
    ...
    ros::Subscriber fsm_sub = nh.subscribe<sunray_msgs::UAVControlFSMState>(
        uav_namespace + "/sunray/uav_control_state", 10, Sunray_FSM_State_Callback);
    while (ros::ok()) {
        ...
        if (current_state == sunray_fsm::SunrayState::INIT) {
            // 如果无人机当前处于LAND状态, 并且两个时间戳都不为0,则说明无人机降落成功，本节点可以结束
            if (time_hover_start != ros::Time(0) && time_ready_start != ros::Time(0)) {
                ros::shutdown();
                return 0;
            }
        }
